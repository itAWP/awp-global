...
async function mutateJsonBlob(pathname, mutator, maxAttempts = 5) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    if (attempt > 0) {
      // A write's own read-your-writes ETag can lag the storage layer by a
      // beat, so an immediate re-read after a conflict can observe the same
      // stale ETag again and never converge. Back off before retrying.
